...
Finally, the results reaffirm the value of a clear separation of concerns in the runtime: \emph{(a)} a calculus that enforces sound local expansions; \emph{(b)} salience that steers instantiation; and \emph{(c)} a guidance interface that orders frontier states. Each layer can evolve independently without reinterpreting previous chapters’ formal commitments (cf.\ Chapter~\ref{cp:systemdesign}).
