...
This appendix explains how to rebuild all experiments reported in Chapter~\ref{ch:experiments} using the two reference implementations in this repository: the Java engine under \path{design_java/} (Design~A) and the Python port under \path{python_port/} (Design~B). The instructions assume the project root \path{/home/ahmed/projects/masters}; adjust the paths if you cloned the repository elsewhere. This thesis is fully versioned at \href{https://github.com/A-Mamdouh/masters-thesis}{github.com/A-Mamdouh/masters-thesis}.
...
\begin{itemize}
  ...
  \item \textbf{RAM.} 16~GB or more recommended (8~GB is sufficient for Java plus moderate Python experiments).
  \item \textbf{Java toolchain.} OpenJDK~17 or later; \texttt{javac} and \texttt{java} must be on \texttt{PATH}.
  \item \textbf{Python toolchain.} Python~3.11+ with \texttt{pip} or \texttt{uv}. The port depends on PyTorch, NumPy, and tqdm. Use the supplied \path{python_port/uv.lock} for deterministic installs via \texttt{uv sync}.
...
\begin{itemize}
  \item \textbf{Design~A:} \path{design_java/}
    \begin{compactitem}
      \item \path{Main.java}: entry point for all Java experiments.
      \item \path{modelGeneration/}: priority queue, tableau rules, heuristics.
      \item \path{ontology/} and \path{stories/}: knowledge graph fragment and benchmark narratives.
      \item Logs follow the filename pattern \texttt{test\_results\_\allowbreak<W>\_\allowbreak{}workers\_\allowbreak<T>\allowbreak{}ms.txt} (for example, \texttt{test\_results\_\allowbreak12\_\allowbreak{}workers\_\allowbreak200ms.txt}).
    \end{compactitem}
  \item \textbf{Design~B:} \path{python_port/}
    \begin{compactitem}
      \item \path{src/nls_python/model_generation/}: tableau port, policies, neural helpers.
      \item \path{data/}: annotated story sets (train.json, val.json, test.json,\\
      java\_design\_test.json).
      \item \path{experiments/policies/}: saved logs (story\_bank\_results.txt,\\
      annotated\_test\_results.txt), summaries and checkpoints.
      \item \path{scripts/}: bash helpers for batch experiments.
    \end{compactitem}
\end{itemize}
...
\end{verbatim}
\begin{compactitem}
  \item Arguments are \texttt{<workers>} and \texttt{<per-iteration-\allowbreak{}timeout-\allowbreak{}ms>}.
  \item Each log block lists the story sentences and heuristic statistics (time in ms, explored models, success flag) with separators for readability.
  \item Aggregated numbers in Table~\ref{tab:policy_runtimes_java} are the arithmetic mean across the five Java story variants included in the file.
\end{compactitem}
...
\end{verbatim}
\begin{compactitem}
  \item Each output file contains one block per story with the same \texttt{HeuristicStatistics} summary used in Chapter~\ref{ch:results}.
  \item The averages quoted in Tables~\ref{tab:policy_python_results}--\ref{tab:cross_family_comparison} are computed from these files (mean across stories per policy).
\end{compactitem}

\subsection{Neural policy training and evaluation}
The port provides a repeatable pipeline: dataset generation, model training, then experiments. Commands use \texttt{uv run} to guarantee the locked dependency set.
...
\begin{itemize}
  \item \textbf{Logging.} Both implementations log individual story blocks followed by aggregate statistics. Keep the raw files (\path{test_results_*.txt} and \path{experiments/policies/*.txt}); the Results chapter cites these exact paths.
